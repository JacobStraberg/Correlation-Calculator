...
pub fn calcCorr(ally: std.mem.Allocator, x: []const f32, y: []const f32) [2]f32 {
    ...
    var xy: []f32 = ally.alloc(f32, array_len) catch @panic("BARF!");
    var xx: []f32 = ally.alloc(f32, array_len) catch @panic("BARF!");
    var yy: []f32 = ally.alloc(f32, array_len) catch @panic("BARF!");
